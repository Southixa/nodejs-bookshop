...
import { ValidateChangePassword, ValidateLogin, ValidateProfile, ValidateRegister } from "../service/validate.js";
...
export default class UserController {
    ...
    static async getAll(req, res) {
        try {
            const user = await Models.User.find({ isActive: true}).select("-password");
    ...
    static async login(req, res) {
        try {
            ...
            return SendSuccess(res, "Password is not Match");
        } catch (error) {
    ...
    static async fortgetPassword(req, res) {
        try {
    ...
    static async updateProfileImage(req, res) {
        try {
          ...
          // const buff = Buffer.from(files.data, "utf-8");
          // console.log(buff);
          const image_url = await UploadImage(path.data, fileLastName);
          console.log(image_url);
          if (!image_url) {
            return SendError400(res, "Error Upload file");
          }
