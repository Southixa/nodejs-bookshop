...
import { SendCreate, SendError400, SendError500, SendSuccess } from "../service/response.js";
...
export default class CategoryController {
    ...
    static async insert(req, res) {
        try {
            const {name} = req.body;
            if(!name) {
                return SendError400(res, "name is required!");
            }
    ...
    static async updateCategory(req, res) {
        try {
            ...
            const category = await Models.Category.findByIdAndUpdate(categoryId,{
                name, image: image_url,
            })
