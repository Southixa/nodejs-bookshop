...
export default class BannerController {
  ...
  static async getAll(req, res) {
    try {
      //const banner = await Models.Banner.find({isActive: true, _id: bannerId,})
      const banner = await Models.Banner.find({isActive: true});
      if(!banner) {
  ...
  static async insert(req, res) {
    try {
      ...
      return SendSuccess(res, SMessage.create, banner);
    } catch (error) {
        console.log(error);
        return SendError500(res, EMessage.serverFaild, error);
    }
